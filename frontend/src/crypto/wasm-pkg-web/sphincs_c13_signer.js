 ...
 * @returns {string}
 */
export function keygen_from_mnemonic(mnemonic, passphrase) {
    let deferred4_0;
    let deferred4_1;
    try {
 ...
 * @returns {string}
 */
export function sign_from_mnemonic(mnemonic, passphrase, message_hex) {
    let deferred5_0;
    let deferred5_1;
    try {
        const ptr0 = passStringToWasm0(mnemonic, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
 ...
 * @returns {string}
 */
export function sign_with_keys(seed_hex, sk_seed_hex, root_hex, message_hex) {
    let deferred6_0;
    let deferred6_1;
    try {
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('sphincs_c13_signer_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
